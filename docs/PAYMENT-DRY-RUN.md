...
2. Send the payment request with a fresh `Idempotency-Key`.
3. Re-send the identical request; confirm the original result is returned and
   no second payment is created.
4. Re-send with a mutated body under the same key; confirm
   `IDEMPOTENCY_CONFLICT`.
5. Simulate a dependency outage; confirm `DEPENDENCY_UNAVAILABLE` and that no
   payment is written.

## Rollback / kill-switch

Idempotency enforcement on the money path is feature-flagged. Disabling the
flag restores the previous behavior; the flag MUST NOT be disabled on mainnet
without a readiness checklist, since doing so re-opens duplicate-payment risk.
